...
In subsection \ref{sec:vtol path planning}, several B-Spline paths were created for the quadplane VTOL to follow. 
...
An example of this is an urban environment with obstacles that include buildings, wires, poles, vehicles, and people. 
The 2D VTOL controller developed in section \ref{ch:VTOL} requires a B-Spline input to function. 
...
In this section, a B-Spline trajectory generation algorithm is developed that plans safe paths for aircraft to follow through obstacle fields. 
...
At a given point along that line, at distance $d$ from the nearest node, a new candidate node is created. 
...
RRT* is a common variation of the RRT algorithm introduced by \cite{varricchioSamplingbasedAlgorithmsOptimal2014}. 
With traditional RRT, there is no effort to reconnect the tree nodes to increase efficiency. 
...
Although this algorithm eventually converges to an optimal path, it does so under great computational burden due to rewiring after each sample. 

A common variation of RRT is to have it generate Dubins paths instead of straight lines. 
For example, in \cite{yangDubinsPathOrientedRapidly2022} a method is presented for Dubins path generation using RRT. 
...
We want to create a trajectory using degree 3 B-Splines with optimized distance and continuity of position, velocity, and acceleration.
...
However, this algorithm does not guaranty that the B-Spline does not intersect obstacles in the environment. 
...
Safe Flight Corridors are defined spaces that are known to be safe for a vehicle to travel within. 
...
If a specified corridor or series of corridors remains completely free of obstacles and the aircraft stays within that corridor or series of corridors for its flight period, no collisions result during that time.
...
The left shape contains an internal or reflex angle that precludes the shape from being convex. 
...
In other words, a point can be determined to be inside or outside a polygon using a simple linear inequality. 
...
A polygon can be represented as a conglomeration of multiple half-planes, where each edge has its own $\hat{n}_i$ and $\vec{r}_i$ vectors. 
...
For this reason, the control points are constrained to either one SFC or the intersection of both SFCs. 
...
If $d$ control points are constrained within the intersection of $SFC_1$ and $SFC_2$, then each sequential convex hull of the control points is within at least one $SFC$. 
...
Given these conditions, a B-Spline, $b(t)$ remains within the set $SFC_i \space \cup \space SFC_{i+1}$ if $d$ sequential control points $\left\{C_j,C_{j+1}, ..., C_{j+d}\right\}$ are constrained to the intersection region $\mathcal{P}_i$ for the transition from $SFC_i$ to $SFC_{i+1}$.
...
If there is an intersection, that means that there is a feasible region such that both linear inequality constraints are met.
...
Fixed-wing aircraft may not follow trajectories that have a tighter turn radius than a particular value, shown in Fig. \ref{fig:MinTurnRadius} .
...
\begin{figure}
    ...
    \caption{Aircraft flying section of Dubins Path with straight lines, and connected with arcs of minimum turning radius. The aircraft is unable to perform the turn for anything with sharper curvature.}
...
However, because the arc is tangent to the outer wall, the radius line is perpendicular to the inner and outer walls on both sides. 
...
One control point is constrained to a single annulus approximation section, as shown in figure \ref{fig:annulus sampled diagram}. 
These sections are termed annulus segments. 
...
The initially generated path produces suboptimal results for an SFC path; the segments are short and take a random path, as there is no RRT* smoothing operation for reasons of computational efficiency. 
...
\begin{figure}
     ...
     \begin{subfigure}[b]{0.45\textwidth}
         ...
         \caption{Complete set of nodes for RRT algorithm on a $2 \times 2$ obstacle field. The intermediate nodes are shown in orange and the original path is shown in green connecting them sequentially. The other plausible paths between existing nodes are shown in yellow. These alternative paths are explored using Dijkstra's algorithm.}
...
CVXPY is a Python package that contains optimization capabilities to allow the user to optimize some objective function subject to specified linear constraints. 
This paper uses the CLARABEL solver. 
The vector of control point components may be optimized. 
A CVXPY variable is defined with shape $(N_{cntPts}, N_{dimensions})$, so there are $N_{cntPts} \times N_{dimensions}$ degrees of freedom or variables that the optimizer can modify. 
...
This equation may be substituted for the linear inequality $Ax < b$ as
...
An intersection between a 2D SFC represented by $A_{SFC,2D} x < b_{SFC,2D}$ can be determined using the Scipy linprog function on the stacked matrices
...
A trajectory generated by this algorithm could represent the trajectory of a robot arm that moves objects without hitting other objects. 
...
In this case, the RRT algorithm took $2.0087$ seconds to generate the new path.
...
However, the minimum distance path does have the most stable velocity.
...
This entails creating rectangular prisms for both obstacles and SFCs instead of 2D rectangles. 
...
A random point is sampled, connected to the tree, and a rectangular prism is drawn around it. 
...
Because $Q$ is a tall, full rank matrix $(3 \times 2)$, a left pseudoinverse of the form $Q^+ = (Q^\top Q)Q^\top$ obtains the best solution for $y$ as
...
The problem with Dijkstra's algorithm is that it selects segment $S_4$ to connect $b_0$ and $b_2$ and bypass $b_1$, where we assume that adding $S_4$ does not violate angular constraints with the previous line segment not shown in this diagram for simplicity. 
...
Implementation of LIAM creates a more robust framework for a better algorithm.
...
There is no guarantee that the control points remain along the inside turning radius. 
...
Real time implementation requires the total algorithm run time to be ideally less than one second.
...
Future work in this area must take into account all such edge cases of these annulus constraints.
