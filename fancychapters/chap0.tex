...
\begin{figure}[htbp]
    ...
    \caption{Four different multicopters. Figure \ref{fig:racingDrone} is kind of a VTOL itself, due to its high vertical speed.}
    \label{fig:Multicopters}
\end{figure}

However, airplanes are much more efficient at traveling long distances at high speeds, with much less energy consumption than copters. A quadcopter uses four rotors with fairly high power consumption each just to stay in the air without any movement, while an airplane uses a relatively low power thruster maintain a high kinetic energy. For a given battery size, an airplane can stay in the air significantly longer than its equivalent rotorcraft. Airplanes have fewer moving parts than rotorcraft, and can glide to safety in case of rotor failure, which lends some inherent advantages to the use of airframes. Some examples of fixed wing UAVs are shown in Figure \ref{fig:FixedWings}.
...
The primary interest of this research has been to study control for a specific type of UAV called a VTOL (vertical takeoff and landing) aircraft. A VTOL is defined as an airplane that can take off and land vertically \cite{Rehan_Akram_Shahzad_Shams_Ali_2022}. That is, they are rotorcraft and airplane hybrids, with airfoils to generate lift and vertical rotors to take off and land vertically. Such hybridization of design allows VTOLs to take advantage of the takeoff and landing capabilities of pure rotorcraft, while also having and airfoil to travel long distances quickly and efficiently. Much research and development is already underway for applications in both civilian and defense spaces. Some examples of existing VTOL craft are shown in Figure \ref{fig:eVTOLExamples}. 
...
\begin{figure}[htbp]
    ...
    \caption{Four different commercial and defense eVTOL aircraft in various stages of development and use. Each design is specialized to the use case and developer's design philosophy. Figures \ref{fig:archer} and \ref{fig:BetaTech} are commercial eVTOL air taxis in development from Archer Aviation and Beta Technologies respectively. The V-22 Osprey in Figure \ref{fig:osprey} is a famous VTOL that has seen extensive development and deployment for the U.S. Military. Figure \ref{fig:zipline} is the Zipline VTOL, which is under development for food and package delivery.}
