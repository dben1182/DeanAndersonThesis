...
If the aerodynamic coefficients for pitch moment ($C_{M_0},C_{M_\alpha},C_{M_q},C_{M_{\delta_e}}$) are not accurate, an incorrect $\delta_e$ may be calculated. If this control input has an error that is too large, then the aircraft may become unstable, which is especially true during the transition phase.

One may also ask how these aerodynamic coefficients are to be calculated to begin with. Without an estimate 
...
The control input signal for elevator $\delta_e$ control is shown in figure \ref{fig:phughoidControlInput} and the control input signals for aileron $\delta_a$ and rudder $\delta_r$ are shown in figure \ref{fig:dutchRollControlInput}. It turns out that the best control input for the elevator we could find was a chirp signal, which is a sine wave of steadily increasing frequency. This corresponded well to the aircraft's phughoid mode oscillation. 
...
If the aircraft is flying in trim conditions
...
It turns out this work has already been done by other 

\subsection{Signal Processing}

A huge problem to solve is the noise problem. Noise is pervasive in all systems including Unmanned Aerial Systems. Noise exists in the form of electronic sensor noise from the Autopilot's Inertial Measurement unit, airspeed sensors. These noise signals were not modeled in the sensor 

The solution is going to be the application of signal processing techniques to obtain an estimate of the onboard noise and to  perform the optimal cancellation techniques for it. 


We did not pursue this further because we know that it is limited. A limitation to our work here is that 
